...
static void sta_home_struct(lv_obj_t * page)
{
    ...
    lv_obj_set_size(contant, UI_HOR_RES, 30);
    lv_obj_set_pos(contant, 0, 0);
    lv_obj_set_style_bg_color(contant, lv_color_hex(0x00FF00), 0);   /* 设容器背景色（注意 API） */
...
static void sta_ui_home(lv_obj_t *page)
{
    ...
    for (int i = 0; i < HOME_PAGE_NUM; i++) {
        ...
        lv_obj_set_size(staUiStruct.homePage[i], UI_HOR_RES, UI_VER_RES-30);
        lv_obj_set_pos(staUiStruct.homePage[i], 0, 30);
        if (i != 0) {
            lv_obj_add_flag(staUiStruct.homePage[i], LV_OBJ_FLAG_HIDDEN);
        }
    }
    sta_ui_home_music(staUiStruct.homePage[0]);
}
